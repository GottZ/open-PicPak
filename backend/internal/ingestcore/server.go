...
// subpath — the noise-404 semantic, preserved); a non-owned path falls through to the SPA (design §2).
// In the standalone process every "/" hit is routed through Handle, so this is a no-op there.
func (s *Server) OwnsPath(r *http.Request) bool {
	parts := strings.Split(strings.Trim(r.URL.Path, "/"), "/")
	return s.token != "" && len(parts) >= 1 && parts[0] == s.token
}
